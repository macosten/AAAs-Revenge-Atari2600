...
; A movement cooldown should exist so you don't need frame-perfect inputs to move the Avinroo.
 ...
 ;The high 4 bytes of d are free.
...
;Floating-point numbers to keep track of the yoyo's movement. Ironically, we have enough RAM for this.
 ...
 const greatestStringOffset = gameOverStringOffset
...
_gameOverLoop
 ...
 if TextIndex > greatestStringOffset then TextIndex = 0 ; Return to the first string if we try to advance past the last one.
